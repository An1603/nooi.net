    ...
    >
      ...
      {ytId ? (
        ...
        <div className="relative aspect-video bg-muted overflow-hidden">
          ...
          {parsed.duration && (
            <span className="absolute bottom-2 right-2 text-[12px] bg-black/70 text-white px-1.5 py-0.5 rounded">
              {parsed.duration}
            </span>
          )}
        </div>
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[12px] text-muted-foreground uppercase tracking-wider">{type.label}</span>
        </div>
        ...
        {/* Category */}
        {parsed.category && (
          <span className="inline-block text-[12px] px-2 py-0.5 rounded-full bg-primary/10 text-primary mt-2">
            {parsed.category}
          </span>
        )}
        ...
        {file_type === "audio" && parsed.url && (
          <div className="mt-2 flex items-center gap-2 text-[12px] text-muted-foreground bg-muted/30 rounded-lg px-3 py-2">
        ...
        {/* PDF badge */}
        {file_type === "pdf" && (
          <div className="mt-2 flex items-center gap-2 text-[12px] text-muted-foreground bg-orange-500/10 rounded-lg px-3 py-2">
        ...
        {/* Date */}
        <div className="flex items-center gap-1.5 text-[12px] text-muted-foreground mt-3 pt-2 border-t border-border/50">
          <Calendar size={12} />
