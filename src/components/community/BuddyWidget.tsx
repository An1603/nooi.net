    ...
    <div className="rounded-xl border border-pink-500/15 bg-gradient-to-br from-pink-500/5 via-card to-red-500/5 p-5 card-elevated">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">Cùng nhau thực hành, nhắc nhở lẫn nhau</p>
          </div>
        </div>
        <button
            ...
            {filteredUsers.slice(0, 10).map(u => (
              <div key={u.user_id} className="flex items-center justify-between p-2 rounded-lg hover:bg-muted/20">
                ...
                <button
                  ...
                  className="text-[10px] bg-pink-500/10 text-pink-400 px-2 py-1 rounded-lg hover:bg-pink-500/20 disabled:opacity-50"
                >
      ...
      {loading ? (
        ...
        <div className="text-center py-6">
          ...
          <p className="text-[10px] text-muted-foreground/60 mt-1">Nhấn &quot;Tìm bạn&quot; để bắt đầu kết nối</p>
        </div>
      ) : (
      ...
      {pendingSent.length > 0 && (
        <div className="mt-3 pt-3 border-t border-border/30">
          <p className="text-[10px] text-muted-foreground">Đã gửi lời mời: {pendingSent.map(p => p.toName).join(", ")}</p>
        </div>
      )}
    </div>
  );
