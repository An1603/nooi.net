  ...
  return (
    <div className="max-w-2xl mx-auto p-6 space-y-8">
      ...
      <button onClick={async () => { 
        const { downloadSvgAsPng } = await import('@/lib/svg-to-png');
        const svg = chartRef.current?.querySelector('svg'); 
        if (svg) downloadSvgAsPng(svg as unknown as SVGSVGElement, 'ket-qua-danh-gia-nooi.png'); 
      }} className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-primary/10 text-primary text-sm font-medium hover:bg-primary/20 transition-colors"><Download className="size-4" /> Tải ảnh kết quả</button>
      <div className="rounded-xl border border-border bg-card p-5 space-y-3">
