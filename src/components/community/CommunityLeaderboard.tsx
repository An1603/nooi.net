...
export default function CommunityLeaderboard() {
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5">
        ...
        {data.slice(0, 10).map((entry) => (
          <div key={entry.rank} className={`flex items-center gap-3 p-2.5 rounded-lg ${
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-muted-foreground">{entry.journals} nhật ký</p>
            </div>
      ...
      {data.length > 10 && (
        <p className="text-[10px] text-center text-muted-foreground mt-3">+{data.length - 10} người khác</p>
      )}
    </div>
  );
}
