              ...
              <tbody className="divide-y divide-border/10">
                {filtered.map(u => <tr key={u.id} className="hover:bg-white/[0.01]">
                  <td className="px-4 py-3">
                    ...
                    <div className="min-w-0"><p className="text-sm font-medium truncate hover:text-primary transition-colors">{u.full_name||"Chưa đặt tên"}</p><p className="text-[12px] text-muted-foreground truncate">{u.email??u.id.slice(0,8)+"..."}</p></div>
                    </Link>
                  </td>
                  <td className="px-4 py-3 hidden sm:table-cell"><div className="flex gap-1 flex-wrap">
                    {u.has_numerology&&<span className="text-[12px] px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-400">TS</span>}
                    {u.has_tuvi&&<span className="text-[12px] px-1.5 py-0.5 rounded bg-red-500/10 text-red-400">TV</span>}
                    {u.has_astrology&&<span className="text-[12px] px-1.5 py-0.5 rounded bg-purple-500/10 text-purple-400">CT</span>}
                    {!u.has_numerology&&!u.has_tuvi&&!u.has_astrology&&<span className="text-[12px] text-muted-foreground">—</span>}
                  </div></td>
                  <td className="px-4 py-3 hidden md:table-cell">
                    <span className={`inline-block text-[12px] font-medium px-2 py-0.5 rounded-full border ${LEVEL_COLORS[u.level] || "bg-gray-500/10 text-gray-400 border-gray-500/20"}`}>
