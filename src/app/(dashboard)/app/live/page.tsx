...
function CountdownTimer({ target }: { target: string }) {
  ...
  return <span className="text-[12px] font-mono tabular-nums">{remaining}</span>;
}
        ...
        <div className="rounded-xl border border-border bg-card p-5 space-y-3 animate-in fade-in">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[12px] text-muted-foreground font-mono">ID:</span>
            ...
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[12px] text-muted-foreground font-mono">🔑</span>
                ...
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      <span className="text-[12px] font-bold px-2 py-0.5 rounded-full bg-primary/10 text-primary">Sắp tới</span>
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 mt-3 text-xs text-muted-foreground">
                      ...
                      {s.meeting_id && (
                        <span className="flex items-center gap-1 font-mono text-[12px] opacity-60 col-span-2">ID: {s.meeting_id}</span>
                      )}
                    </div>
                    ...
                    {isRegistered && s.link && (
                      <div className="mt-3 p-3 rounded-lg bg-primary/10 border border-primary/20 space-y-1.5">
                        <div className="flex items-center justify-between">
                          <span className="text-[12px] font-medium text-primary flex items-center gap-1"><Lock className="w-3 h-3" /> Thông tin phòng học</span>
                          <div className="flex gap-2">
                            <a href={icsUrl} download={`nooi-${s.meeting_id || "live"}.ics`} className="text-[12px] text-primary hover:underline flex items-center gap-0.5">
                  ...
                  <div className="flex sm:flex-col sm:items-end items-stretch gap-2 sm:gap-2 sm:shrink-0">
                    ...
                    {!isRegistered && (
                      <button
                        ...
                        className="flex-1 sm:flex-none text-[12px] text-muted-foreground hover:text-primary flex items-center justify-center gap-1 disabled:opacity-50 sm:justify-end"
                      >
              ...
              <div key={s.id} className="rounded-lg border border-border/50 bg-card/50 p-4 opacity-70">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="text-[12px] px-2 py-0.5 rounded-full bg-muted/30 text-muted-foreground shrink-0">Đã kết thúc</span>
                    <h3 className="text-sm font-medium truncate">{s.title}</h3>
                  </div>
                  <span className="text-[12px] text-muted-foreground shrink-0 ml-2">{s.date} · {s.time}</span>
                </div>
                {s.registered > 0 && (
                  <p className="text-[12px] text-muted-foreground mt-1">{s.registered} học viên đã tham gia</p>
                )}
              </div>
            ))}
          </div>
